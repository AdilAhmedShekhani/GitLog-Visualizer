...
function runGitVizCmdWithFlags(flagsStr) {
	...
	try {
		...
		if (rawOutput.startsWith("GitViz")) {
			console.warn(
				`The provided command/flags aren't valid!, see: ${command}`
			);
			process.exit(1);
		}
...
function colorize(text, colorCode, enabled = true) {
	...
	return enabled
		? `${ANSI_CODES[colorCode]}${text}${ANSI_CODES.reset}`
		: text;
}
...
function renderTable(rowMatrix, columnDefinitions, colorEnabled = true) {
	...
	const headerRow = columnDefinitions
		.map((col) =>
			colorize(padCell(col.header, col.width, col.align), "bold")
		)
		.join(" ");
	const dividerRow = columnDefinitions
		.map((col) => "─".repeat(Math.max(0, col.width)))
		.join(" ");
...
function calcHighestWidth(obj, objSelector, min = 0) {
	...
	return values.length > 0
		? Math.max(...values.map((v) => stripAnsi(v).length), min)
		: min;
}
...
function printSectionHeader(title) {
	console.log(colorize(centerText(` ${title} `), "cyan"));
...
function showMeta(metaObj) {
	console.log(`
${colorize("Repo:", "gray")} ${colorize(
		metaObj.repo === "." ? ". (current directory)" : metaObj.repo,
		"cyan"
	)}
${colorize("Time Range:", "gray")} ${colorize(
		metaObj.since,
		"yellow"
	)} to ${colorize(metaObj.until, "yellow")}, ${colorize(
		metaObj.rangeDays,
		"magenta"
	)} ${colorize("days", "gray")}
${colorize("First commit was created at", "gray")} ${colorize(
		metaObj.firstCommitDate,
		"green"
	)}, ${colorize(metaObj.repoAgeDays, "magenta")} ${colorize(
		"days ago",
		"gray"
	)}
${colorize("This dashboard is generated at", "gray")} ${colorize(
		metaObj.generated,
		"pink"
	)}
`);
...
function showContributors(contributorsObj, contributorsStatsObj) {
	...
	const contributorRows = computeContributorPercentages(combinedObjArr).map(
		(c) => [
			colorize(`${c.name} <${c.email}>`, "blue"),
			colorize(c.commits, "yellow"),
			colorize(c.percent + "%", "magenta"),
			colorize(c.additions, "green"),
			colorize(c.deletions, "red"),
		]
	);
...
function showFileStats(fileObj) {
	const sortedFileObj = fileObj
		.sort((a, b) => b.additions - a.additions)
		.slice(0, CONFIG.FILES_LIMIT);

	const fileRows = sortedFileObj.map((file) => [
		colorize(file.path, "cyan"),
		colorize(file.changes, "gray"),
		colorize(file.additions, "green"),
		colorize(file.deletions, "red"),
	]);
...
function showDirectoryStats(directoryObj) {
	const sortedDirectoryObj = directoryObj
		.sort((a, b) => b.additions - a.additions)
		.slice(0, CONFIG.DIRECTORY_LIMIT);

	const directoryRows = sortedDirectoryObj.map((directory) => [
		colorize(directory.directory, "magenta"),
		colorize(directory.changes, "gray"),
		colorize(directory.additions, "green"),
		colorize(directory.deletions, "red"),
	]);
...
function showBranches(branchObj, branchStatsObj) {
	...
	const branchesRaw = combinedObjArr.map((branchStat) => [
		...
		colorize(
			`${truncateText(branchStat.lastAuthor, 12)}/${branchStat.lastCommitDate
			} `,
			"cyan"
		),
	...
	console.log(
		renderTable(branchesRaw, [
			{
				header: colorize("Branch", "pink"),
				width: calcHighestWidth(branchesRaw, 0, 11),
				align: "left",
			},
			{ header: colorize("Tip Hash", "gray"), width: 8 },
			{
				header: colorize("Commits", "yellow"),
				width: calcHighestWidth(branchesRaw, 2, 8),
				align: "right",
			},
			{
				header: colorize("Merges", "green"),
				width: calcHighestWidth(branchesRaw, 3, 6),
				align: "right",
			},
			{
				header: colorize("Last Commit Author/Date", "cyan"),
				width: calcHighestWidth(branchesRaw, 4, 23),
			},
			{
				header: colorize("Authors", "magenta"),
				width: calcHighestWidth(branchesRaw, 5, 7),
				align: "right",
			},
		])
	);
